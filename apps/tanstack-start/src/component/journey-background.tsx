...
function Wrap({ preview, children }: { preview: boolean; children: React.ReactNode }) {
...
export function BackgroundV1({ preview = false }: { preview?: boolean }) {
  ...
  const c = dark
    ? { p1:"#4f46e5", p2:"#7c3aed", p3:"#0ea5e9", rej:"#ef4444", fin:"#22c55e", thorn:"rgba(255,255,255,0.55)" }
    : { p1:"#3730a3", p2:"#6d28d9", p3:"#0369a1", rej:"#dc2626", fin:"#15803d", thorn:"rgba(15,23,42,0.55)" };
  ...
  const thorns: number[][] = [
    [175,866,169,854,181,854],[255,857,249,845,261,845],
    [338,847,332,835,344,835],[430,824,424,812,436,812],
    [476,812,468,808,476,800],[458,740,446,736,458,726],
    [412,718,400,714,412,704],[352,690,340,686,352,676],
    [364,648,356,640,368,636],[408,618,400,610,412,606],
    [464,604,456,596,468,592],[528,594,520,586,532,582],
    [578,588,572,578,582,576],[616,556,608,546,620,542],
    [624,534,616,524,628,520],[612,506,604,496,616,492],
    [602,478,594,468,606,464],[544,450,536,440,548,436],
    [524,436,516,426,528,422],[586,410,578,400,590,396],
    [634,396,626,386,638,382],[696,382,688,372,700,368],
    [742,374,734,364,746,360],[800,366,792,356,804,352],
    [848,348,840,338,852,334],[872,320,864,310,876,306],
    [876,292,868,282,880,278],[850,270,842,260,854,256],
    [846,256,838,246,850,242],[794,242,786,232,798,228],
    [752,226,744,216,756,212],[756,210,748,200,760,196],
    [790,162,782,152,794,148],[848,152,840,142,852,138],
    [912,142,904,132,916,128],[988,134,980,124,992,120],
    [1060,128,1052,118,1064,114],[1134,116,1126,106,1138,102],
    [1202,100,1194,90,1206,86],[1266,80,1258,70,1270,66],
  ];

  const milestones = [
    { cx:100,  cy:870, r:8,   label:"Start",          dx:14, dy:5,   col:c.p1, op:0.55 },
    { cx:420,  cy:820, r:5,   label:"Polish CV",       dx:12, dy:5,   col:c.p2, op:msOp },
    { cx:360,  cy:680, r:5,   label:"Job Boards",      dx:12, dy:5,   col:c.p2, op:msOp },
    { cx:600,  cy:580, r:5,   label:"First Reply",     dx:12, dy:5,   col:c.p2, op:msOp },
    { cx:610,  cy:492, r:5,   label:"Recruiter Call",  dx:12, dy:5,   col:c.p2, op:msOp },
    { cx:688,  cy:380, r:5,   label:"Phone Screen",    dx:12, dy:5,   col:c.p2, op:msOp },
    { cx:842,  cy:342, r:5,   label:"Tech Interview",  dx:12, dy:5,   col:c.p2, op:msOp },
    { cx:844,  cy:258, r:5,   label:"System Design",   dx:12, dy:5,   col:c.p2, op:msOp },
    { cx:880,  cy:148, r:5,   label:"Final Round",     dx:12, dy:5,   col:c.p2, op:msOp },
    { cx:1128, cy:112, r:5,   label:"Ref. Check",      dx:12, dy:5,   col:c.p3, op:msOp },
    { cx:1322, cy:52,  r:10,  label:"Offer! 🎉",        dx:-76,dy:-13, col:c.fin,op:0.65,glow:true },
  ];

  const deadEnds = [
    { d:"M 420 820 C 510 844,586 868,612 896",           lx:536, ly:888, label:"rejected"      },
    { d:"M 600 580 C 684 566,762 552,832 534",           lx:760, ly:530, label:"no reply"      },
    { d:"M 840 342 C 910 318,968 302,994 274 C 1014 256,1008 238,992 228", lx:962,ly:272,label:"not this time" },
    { d:"M 1128 112 C 1174 88,1216 72,1234 46",          lx:1160,ly:44,  label:"so close..."   },
  ];
    ...
    <Wrap preview={preview}>
      <svg {...SVG}>
        <defs>
          ...
          <filter id={gId("v1end")} x="-80%" y="-80%" width="260%" height="260%">
            <feGaussianBlur in="SourceGraphic" stdDeviation="7" result="b" />
            <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge>
          </filter>
          <filter id={gId("v1rej")} x="-30%" y="-30%" width="160%" height="160%">
        ...
        <path
          ...
          stroke={`url(#${gId("v1g")})`}
          strokeWidth="8" strokeLinecap="round" opacity={glowOp}
        ...
        <path
          d="M 100 870 C 220 860,340 845,420 820 C 500 795,520 765,480 740
             ...
             C 960 132,1060 132,1128 112 C 1188 94,1244 74,1322 52"
          stroke={`url(#${gId("v1g")})`}
          strokeWidth="2.5" strokeDasharray="12 6" strokeLinecap="round"
          opacity={pathOp}
        />
        ...
        {deadEnds.map((b, i) => (
          <g key={i}>
            <path d={b.d} stroke={c.rej} strokeWidth="2" strokeDasharray="5 5"
              strokeLinecap="round" opacity={rejOp} filter={`url(#${gId("v1rej")})`} />
            <text x={b.lx} y={b.ly} fontSize="9" fill={c.rej} opacity={rejOp * 0.8}
        ...
        {thorns.map((t, i) => (
          <g key={i}>
            <line x1={t[0]} y1={t[1]} x2={t[2]} y2={t[3]} stroke={c.thorn}
              strokeWidth="1.3" strokeLinecap="round" opacity={thornOp} />
            <line x1={t[0]} y1={t[1]} x2={t[4]} y2={t[5]} stroke={c.thorn}
              strokeWidth="1.3" strokeLinecap="round" opacity={thornOp} />
          </g>
        ))}
          ...
          <g key={i}>
            {glow && (
              <circle cx={cx} cy={cy} r={r + 10} fill="none"
                stroke={col} strokeWidth="1" opacity={op * 0.3}
                filter={`url(#${gId("v1end")})`} />
            )}
            <circle cx={cx} cy={cy} r={r + 4} fill="none"
              stroke={col} strokeWidth="1" opacity={op * 0.25} />
            <circle cx={cx} cy={cy} r={r} fill={col} opacity={op}
              filter={glow ? `url(#${gId("v1end")})` : undefined} />
            <text x={cx + dx} y={cy + dy} fontSize="10" fill={dark?"white":"#0f172a"}
              opacity={op * 0.6} fontFamily="monospace"
              fontWeight={glow ? "bold" : "normal"}>{label}</text>
          </g>
        ))}
      </svg>
    </Wrap>
...
export function BackgroundV2({ preview = false }: { preview?: boolean }) {
  ...
  const nodes = [
    ...
    {x:60,y:850},{x:160,y:840},{x:270,y:860},{x:390,y:845},{x:510,y:855},
    {x:640,y:840},{x:770,y:850},{x:900,y:838},{x:1040,y:845},{x:1180,y:840},
    {x:1310,y:848},{x:1400,y:840},
    // row 2
    {x:100,y:740},{x:230,y:755},{x:380,y:738},{x:530,y:750},{x:680,y:735},
    {x:830,y:748},{x:980,y:732},{x:1130,y:742},{x:1280,y:730},{x:1390,y:738},
    // row 3
    {x:60,y:630},{x:200,y:648},{x:370,y:625},{x:550,y:640},{x:730,y:618},
    {x:910,y:632},{x:1080,y:615},{x:1240,y:628},{x:1380,y:620},
    // row 4
    {x:140,y:510},{x:320,y:528},{x:510,y:505},{x:700,y:520},{x:890,y:500},
    {x:1060,y:515},{x:1220,y:498},{x:1370,y:510},
    // row 5
    {x:80,y:390},{x:270,y:410},{x:470,y:385},{x:670,y:400},{x:870,y:378},
    {x:1050,y:395},{x:1230,y:375},{x:1390,y:385},
    // row 6
    {x:160,y:270},{x:380,y:290},{x:610,y:265},{x:840,y:282},{x:1060,y:260},
    {x:1260,y:278},{x:1400,y:265},
    // top — sparse success zone
    {x:260,y:148},{x:540,y:132},{x:820,y:148},{x:1060,y:128},{x:1280,y:140},
    {x:1400,y:130},
  ];
      ...
      <svg {...SVG}>
        <defs>
          <radialGradient id={gId("v2bg")} cx="75%" cy="15%" r="70%">
            <stop offset="0%"   stopColor={dark?"#1e1b4b":"#ede9fe"} stopOpacity={dark?0.6:0.5} />
            <stop offset="100%" stopColor={dark?"#000000":"#ffffff"} stopOpacity="0" />
          </radialGradient>
          <filter id={gId("v2glow")} x="-60%" y="-60%" width="220%" height="220%">
            <feGaussianBlur in="SourceGraphic" stdDeviation="5" result="b"/>
            <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge>
          </filter>
          <filter id={gId("v2hub")} x="-80%" y="-80%" width="260%" height="260%">
        ...
        {hubs.map(({ x, y, label }, i) => {
          ...
          return (
            <g key={i} filter={`url(#${gId(isEnd ? "v2hub" : "v2glow")})`}>
              <circle cx={x} cy={y} r={isEnd ? 20 : 14} fill="none"
                stroke={col} strokeWidth="1" opacity={hubOp * 0.3} />
              <circle cx={x} cy={y} r={isEnd ? 12 : 8} fill="none"
                stroke={col} strokeWidth="1" opacity={hubOp * 0.5} />
              <circle cx={x} cy={y} r={isEnd ? 6 : 4}
                fill={col} opacity={isEnd ? 0.75 : hubOp} />
              <text x={x + (isEnd ? -60 : 12)} y={y + (isEnd ? -14 : 5)}
                fontSize={isEnd ? "11" : "10"} fill={dark?"white":"#0f172a"}
                opacity={isEnd ? 0.55 : 0.3} fontFamily="monospace"
                fontWeight={isEnd ? "bold" : "normal"}>{label}</text>
            </g>
          );
        })}
      </svg>
      ...
      <svg {...SVG}>
        <defs>
          <linearGradient id={gId("v3rej")} x1="0%" y1="100%" x2="0%" y2="0%">
            <stop offset="0%"   stopColor={dark?"#6366f1":"#4f46e5"} stopOpacity="0.6" />
            <stop offset="100%" stopColor={rejectCol} stopOpacity="0.8" />
          </linearGradient>
          <linearGradient id={gId("v3win")} x1="0%" y1="100%" x2="0%" y2="0%">
            <stop offset="0%"   stopColor={dark?"#6366f1":"#4f46e5"} stopOpacity="0.6" />
            <stop offset="40%"  stopColor={dark?"#a855f7":"#7c3aed"} stopOpacity="0.8" />
            <stop offset="100%" stopColor={successCol} stopOpacity="1" />
          </linearGradient>
          <filter id={gId("v3glow")} x="-20%" y="-20%" width="140%" height="140%">
            <feGaussianBlur in="SourceGraphic" stdDeviation="4" result="b"/>
            <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge>
          </filter>
          <filter id={gId("v3win")} x="-30%" y="-30%" width="160%" height="160%">
        ...
        {/* Rejected ribbons */}
        {rejected.map((seg, i) => {
          const [sx,sy,cp1x,cp1y,cp2x,cp2y,ex,ey] = seg as [number,number,number,number,number,number,number,number];
          return (
            <g key={i}>
              <path
                d={`M ${sx} ${sy} C ${cp1x} ${cp1y}, ${cp2x} ${cp2y}, ${ex} ${ey}`}
                stroke={`url(#${gId("v3rej")})`}
                strokeWidth="1.5" strokeLinecap="round" opacity={ribbonOp}
              />
              <line x1={ex-5} y1={ey-5} x2={ex+5} y2={ey+5}
                stroke={rejectCol} strokeWidth="1.5" strokeLinecap="round"
        ...
        <path
          d="M 600 900 C 580 740, 560 560, 620 420 C 680 280, 820 180, 980 120 C 1100 72, 1240 52, 1360 30"
          stroke={`url(#${gId("v3win")})`}
          strokeWidth="2.5" strokeLinecap="round" opacity={successOp}
        />

        {/* Finish glow */}
        <circle cx={1360} cy={30} r={10} fill={successCol} opacity={0.7}
          filter={`url(#${gId("v3glow")})`} />
        <text x={1280} y={22} fontSize="11" fill={successCol} opacity={0.65}
          fontFamily="monospace" fontWeight="bold">Offer! 🎉</text>

        {/* Label at bottom */}
        <text x={40} y={880} fontSize="10" fill={dark?"white":"#0f172a"}
          opacity={0.15} fontFamily="monospace">Every line is an attempt.</text>
      </svg>
    </Wrap>
  );
}
...
export function BackgroundV4({ preview = false }: { preview?: boolean }) {
  ...
  // Build hex data
  type HexCell = { cx: number; cy: number; norm: number; label?: string };
  const hexes: HexCell[] = [];
  for (let col = 0; col < COLS; col++) {
    for (let row = 0; row < ROWS; row++) {
      ...
      const dx = 1440 - cx, dy = cy;          // dy positive = farther from top
      const dist = Math.sqrt(dx * dx + dy * dy);
    ...
    <Wrap preview={preview}>
      <svg {...SVG}>
        <defs>
          ...
          <filter id={gId("v4fin")} x="-80%" y="-80%" width="260%" height="260%">
            <feGaussianBlur in="SourceGraphic" stdDeviation="7" result="b"/>
            <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge>
          </filter>
          {/* Radial gradient: bright at top-right, dim at bottom-left */}
          <radialGradient id={gId("v4rg")} cx="100%" cy="0%" r="90%">
            <stop offset="0%"   stopColor={dark?"#1e1b4b":"#c7d2fe"} stopOpacity={dark?0.5:0.4} />
            <stop offset="100%" stopColor={dark?"#000000":"#ffffff"} stopOpacity="0" />
          </radialGradient>
        </defs>
        ...
        {hexes.map(({ cx, cy, norm, label }, i) => {
          ...
          const borderOp = dark
            ...
            : (0.07 + norm * 0.18);
          const fillOp = isNear
            ? (dark ? 0.1 + norm * 0.1 : 0.06 + norm * 0.1)
            : 0;

          return (
            <g key={i} filter={isMilestone ? `url(#${gId("v4ms")})` : undefined}>
              <polygon
                points={hexPts(cx, cy)}
...
export function BackgroundV5({ preview = false }: { preview?: boolean }) {
  ...
  const c = dark
    ? { p1:"#4f46e5", p2:"#818cf8", p3:"#7c3aed", p4:"#0ea5e9", rej:"#f87171", fin:"#22c55e", thorn:"rgba(255,255,255,0.5)", node:"255,255,255" }
    : { p1:"#3730a3", p2:"#6366f1", p3:"#6d28d9", p4:"#0369a1", rej:"#ef4444", fin:"#15803d", thorn:"rgba(15,23,42,0.5)",    node:"15,23,42"    };
  ...
  type HexCell = { cx: number; cy: number; norm: number };
  const hexes: HexCell[] = [];
  for (let col = 0; col < COLS; col++) {
    for (let row = 0; row < ROWS; row++) {
      const cx = col * HW;
  ...
  const nodes = [
    {x:80,y:820},{x:200,y:800},{x:340,y:815},{x:490,y:800},{x:640,y:810},
    {x:790,y:798},{x:950,y:808},{x:1110,y:795},{x:1270,y:802},
    {x:140,y:690},{x:300,y:675},{x:470,y:688},{x:640,y:672},{x:810,y:681},
    {x:980,y:668},{x:1140,y:675},{x:1310,y:662},
    {x:60,y:555},{x:240,y:542},{x:430,y:558},{x:620,y:540},{x:810,y:552},
    {x:1000,y:535},{x:1175,y:548},{x:1350,y:530},
    {x:170,y:415},{x:380,y:428},{x:590,y:410},{x:790,y:422},{x:990,y:405},
    {x:1170,y:418},{x:1360,y:400},
    {x:100,y:275},{x:330,y:290},{x:570,y:272},{x:800,y:285},{x:1020,y:265},
    {x:1230,y:278},{x:1400,y:260},
    {x:240,y:140},{x:530,y:125},{x:820,y:138},{x:1060,y:118},{x:1300,y:130},
  ];
  ...
  const thorns: number[][] = [
    [210,863,204,851,216,851],[295,849,289,837,301,837],
    [368,833,362,821,374,821],[430,820,424,808,436,808],
    [460,740,448,736,460,726],[400,706,388,702,400,692],
    [358,680,346,676,358,666],[408,620,400,612,412,608],
    [470,605,462,597,474,593],[548,592,540,584,552,580],
    [604,576,596,566,608,562],[618,510,610,500,622,496],
    [608,486,600,476,612,472],[548,445,540,435,552,431],
    [524,432,516,422,528,418],[594,408,586,398,598,394],
    [644,392,636,382,648,378],[700,378,692,368,704,364],
    [750,370,742,360,754,356],[808,360,800,350,812,346],
    [850,344,842,334,854,330],[870,318,862,308,874,304],
    [876,290,868,280,880,276],[848,262,840,252,852,248],
    [790,242,782,232,794,228],[754,218,746,208,758,204],
    [790,162,782,152,794,148],[856,148,848,138,860,134],
    [920,138,912,128,924,124],[992,128,984,118,996,114],
    [1066,118,1058,108,1070,104],[1140,106,1132,96,1144,92],
    [1210,88,1202,78,1214,74],[1272,68,1264,58,1276,54],
  ];

  const milestones = [
    { cx:100,  cy:870, r:8,  label:"Start",         dx:14, dy:5,  col:c.p1, op:0.55 },
    { cx:420,  cy:820, r:5,  label:"Polish CV",      dx:12, dy:5,  col:c.p2, op:0.38 },
    { cx:360,  cy:678, r:5,  label:"Job Boards",     dx:12, dy:5,  col:c.p2, op:0.38 },
    { cx:600,  cy:578, r:5,  label:"First Reply",    dx:12, dy:5,  col:c.p2, op:0.38 },
    { cx:612,  cy:490, r:5,  label:"Recruiter Call", dx:12, dy:5,  col:c.p3, op:0.38 },
    { cx:692,  cy:378, r:5,  label:"Phone Screen",   dx:12, dy:5,  col:c.p3, op:0.38 },
    { cx:844,  cy:340, r:5,  label:"Tech Interview", dx:12, dy:5,  col:c.p3, op:0.38 },
    { cx:846,  cy:256, r:5,  label:"System Design",  dx:12, dy:5,  col:c.p3, op:0.38 },
    { cx:882,  cy:144, r:5,  label:"Final Round",    dx:12, dy:5,  col:c.p4, op:0.38 },
    { cx:1130, cy:110, r:5,  label:"Ref. Check",     dx:12, dy:5,  col:c.p4, op:0.38 },
    { cx:1324, cy:50,  r:10, label:"Offer! 🎉",       dx:-76,dy:-14,col:c.fin,op:0.7, glow:true },
  ];

  const deadEnds = [
    { d:"M 420 820 C 500 840,576 866,604 894",            lx:530, ly:888, label:"rejected"      },
    { d:"M 600 578 C 680 562,758 546,828 526",            lx:758, ly:524, label:"no reply"      },
    { d:"M 844 340 C 912 316,970 300,996 272 C 1016 254,1010 236,994 226", lx:964,ly:270,label:"not this time" },
    { d:"M 1130 110 C 1176 86,1218 70,1236 44",           lx:1162,ly:42,  label:"so close..."   },
  ];
      ...
      <svg {...SVG}>
        <defs>
          ...
          <radialGradient id={gId("v5bg")} cx="80%" cy="5%" r="75%">
            <stop offset="0%"   stopColor={dark?"#1e1b4b":"#c7d2fe"} stopOpacity={dark?0.45:0.35} />
            <stop offset="100%" stopColor={dark?"#000000":"#ffffff"} stopOpacity="0" />
          </radialGradient>
          <filter id={gId("v5blur")} x="-5%" y="-5%" width="110%" height="110%">
            <feGaussianBlur in="SourceGraphic" stdDeviation="4" />
          </filter>
          <filter id={gId("v5glow")} x="-60%" y="-60%" width="220%" height="220%">
            <feGaussianBlur in="SourceGraphic" stdDeviation="5" result="b"/>
            <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge>
          </filter>
          <filter id={gId("v5fin")} x="-80%" y="-80%" width="260%" height="260%">
            <feGaussianBlur in="SourceGraphic" stdDeviation="8" result="b"/>
            <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge>
          </filter>
        </defs>
        ...
        {hexes.map(({ cx, cy, norm }, i) => {
          ...
          return (
            <polygon key={`h${i}`}
              points={hexPts(cx, cy)}
              stroke={c.p1} strokeWidth="0.7"
              fill={dark ? "#312e81" : "#e0e7ff"}
              fillOpacity={fOp} opacity={bOp}
            />
          );
        })}
        ...
        {ribbons.map(([sx,sy,cp1x,cp1y,cp2x,cp2y,ex,ey], i) => (
          <g key={`r${i}`}>
            <path d={`M ${sx} ${sy} C ${cp1x} ${cp1y}, ${cp2x} ${cp2y}, ${ex} ${ey}`}
              stroke={`url(#${gId("v5rg")})`}
              strokeWidth="1.2" strokeLinecap="round" opacity={ribOp} />
            <line x1={ex-4} y1={ey-4} x2={ex+4} y2={ey+4}
              stroke={c.rej} strokeWidth="1.4" strokeLinecap="round" opacity={ribOp * 1.8} />
            <line x1={ex+4} y1={ey-4} x2={ex-4} y2={ey+4}
              stroke={c.rej} strokeWidth="1.4" strokeLinecap="round" opacity={ribOp * 1.8} />
          </g>
        ))}
        ...
        {edges.map(([x1,y1,x2,y2,norm], i) => (
          <line key={`e${i}`} x1={x1} y1={y1} x2={x2} y2={y2}
            stroke={`rgb(${c.node})`} strokeWidth="0.8"
        ...
        <path
          ...
          opacity={glowOp} filter={`url(#${gId("v5blur")})`}
        />
        ...
        {deadEnds.map((b, i) => (
          <g key={`d${i}`}>
            <path d={b.d} stroke={c.rej} strokeWidth="1.8"
              strokeDasharray="5 5" strokeLinecap="round" opacity={rejOp} />
            <text x={b.lx} y={b.ly} fontSize="9" fill={c.rej}
              opacity={rejOp * 0.75} fontFamily="monospace">{b.label}</text>
          </g>
        ))}
        ...
        {thorns.map((t, i) => (
          <g key={`t${i}`}>
            <line x1={t[0]} y1={t[1]} x2={t[2]} y2={t[3]}
              stroke={c.thorn} strokeWidth="1.2" strokeLinecap="round" opacity={thornOp} />
        ...
        {/* ── Layer 4e: Milestones ── */}
        {milestones.map(({ cx, cy, r, label, dx, dy, col, op, glow }, i) => (
          <g key={`m${i}`} filter={glow ? `url(#${gId("v5fin")})` : `url(#${gId("v5glow")})`}>
            <circle cx={cx} cy={cy} r={r + 5} fill="none"
              stroke={col} strokeWidth="1" opacity={op * 0.2} />
            <circle cx={cx} cy={cy} r={r} fill={col} opacity={op} />
            <text x={cx + dx} y={cy + dy} fontSize="10"
              fill={dark ? "white" : "#0f172a"}
              opacity={op * 0.65} fontFamily="monospace"
              fontWeight={glow ? "bold" : "normal"}>{label}</text>
          </g>
        ))}
      </svg>
...
export function BackgroundV6({ preview = false }: { preview?: boolean }) {
  ...
  const branches: [number,number,number,number][] = [
    ...
    [80,  820, 80,  730], [200, 820, 200, 640], [340, 820, 340, 730],
    [80,  730, 200, 730], [200, 730, 340, 730],
    [340, 730, 340, 640], [480, 820, 480, 640],
    [340, 640, 480, 640], [480, 640, 620, 640],
    [80,  640, 200, 640], [200, 550, 340, 550],
    [480, 730, 620, 730], [620, 730, 620, 640],
    [620, 550, 760, 550], [760, 640, 760, 550],
    [760, 640, 900, 640],
    // Mid zone
    [480, 550, 620, 550], [480, 460, 620, 460],
    [620, 460, 760, 460], [760, 460, 760, 370],
    [760, 370, 900, 370], [900, 460, 900, 370],
    [900, 550, 1040, 550],[1040, 550, 1040, 460],
    [900, 280, 1040, 280],[1040, 370, 1040, 280],
    [1040, 460, 1180, 460],
    // Top-right zone — sparser, signal converges
    [1040, 280, 1180, 280],[1180, 370, 1180, 280],
    [1180, 280, 1320, 280],[1320, 370, 1320, 280],
    [1180, 190, 1320, 190],[1320, 280, 1320, 190],
    [1040, 190, 1180, 190],[900, 190, 1040, 190],
    [900,  280, 900,  190],[760,  280, 900,  280],
    [760,  280, 760,  190],[620,  280, 760,  280],
    [620,  190, 760,  190],[480,  280, 620,  280],
    [480,  190, 620,  190],[340,  280, 480,  280],
    [200,  280, 340,  280],[80,   280, 200,  280],
    [80,   370, 80,   280],[200,  370, 200,  280],
    // Near-finish convergence
    [1180, 100, 1320, 100],[1320, 190, 1320, 100],
    [1040, 100, 1180, 100],[900,  100, 1040, 100],
  ];
  ...
  return (
    <Wrap preview={preview}>
      <svg {...SVG}>
        <defs>
          <filter id={gId("v6glow")} x="-40%" y="-40%" width="180%" height="180%">
            <feGaussianBlur in="SourceGraphic" stdDeviation="3" result="b"/>
            <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge>
          </filter>
          <filter id={gId("v6fin")} x="-80%" y="-80%" width="260%" height="260%">
            <feGaussianBlur in="SourceGraphic" stdDeviation="7" result="b"/>
            <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge>
          </filter>
          <radialGradient id={gId("v6bg")} cx="90%" cy="0%" r="80%">
            <stop offset="0%"   stopColor={dark?"#1e1b4b":"#eef2ff"} stopOpacity={dark?0.5:0.4}/>
        ...
        {branches.map(([x1,y1,x2,y2], i) => {
          ...
          return (
            <line key={`br${i}`}
              ...
              stroke={traceCol} strokeWidth="1.6"
              strokeLinecap="square" opacity={op}
            />
          );
        })}
          ...
          return (
            <g key={`pd${i}`}>
              ...
              <circle cx={cx} cy={cy} r={6} fill="none"
                stroke={padCol} strokeWidth="1" opacity={op * 0.6} />
              {/* Inner filled pad */}
              <circle cx={cx} cy={cy} r={3}
                fill={padCol} opacity={op} />
            </g>
          );
        })}
        ...
        {chips.map(({ cx, cy, label, w, isFinish }, i) => {
          ...
          const filt  = isFinish ? `url(#${gId("v6fin")})` : `url(#${gId("v6glow")})`;
          const norm  = (cx / 1440 + (1 - cy / 900)) / 2;
          const op    = isFinish ? 0.85 : (dark ? 0.35 + norm * 0.4 : 0.3 + norm * 0.4);
          const pinsY = Array.from({ length: numPins }, (_, k) =>
            cy - ((numPins - 1) * pinGap) / 2 + k * pinGap
          );
          ...
          return (
            <g key={`chip${i}`} filter={filt} opacity={op}>
              ...
              {pinsY.map((py, k) => (
                <line key={`lp${k}`}
                  x1={cx - w / 2 - pinLen} y1={py}
                  x2={cx - w / 2}          y2={py}
                  stroke={bord} strokeWidth="1.5" strokeLinecap="round"
                />
              ))}
              {/* Right pins */}
              {pinsY.map((py, k) => (
                <line key={`rp${k}`}
                  x1={cx + w / 2}          y1={py}
                  x2={cx + w / 2 + pinLen} y2={py}
                  stroke={bord} strokeWidth="1.5" strokeLinecap="round"
                />
              ))}
              {/* Chip body */}
              <rect
                x={cx - w / 2} y={cy - chipH / 2}
                width={w} height={chipH}
                rx={3} ry={3}
                fill={bg} stroke={bord} strokeWidth="1.2"
              />
              {/* Notch (IC orientation mark) */}
              <path
                d={`M ${cx - 4} ${cy - chipH / 2} A 4 4 0 0 1 ${cx + 4} ${cy - chipH / 2}`}
                fill="none" stroke={bord} strokeWidth="1"
              />
              {/* Label */}
              <text x={cx} y={cy + 4}
                textAnchor="middle"
...
export function BackgroundV7({ preview = false }: { preview?: boolean }) {
  ...
  function fieldAngle(x: number, y: number): number {
    // Organic turbulence
    const noise = Math.sin(x * FREQ) * Math.cos(y * FREQ * 1.3) * Math.PI * 0.9
  ...
  type Particle = { pts: [number, number][] };
      ...
      <svg {...SVG}>
        <defs>
          ...
          <radialGradient id={gId("v7bg")} cx="88%" cy="4%" r="65%">
            <stop offset="0%"   stopColor={dark?"#1e1b4b":"#ddd6fe"} stopOpacity={dark?0.55:0.4}/>
            <stop offset="100%" stopColor={dark?"#000":"#fff"}        stopOpacity="0"/>
          </radialGradient>

          {/* Goal glow */}
          <filter id={gId("v7fin")} x="-100%" y="-100%" width="300%" height="300%">
        ...
        {particles.map((p, i) => {
          ...
          return (
            <g key={i}>
              {isHot && (
                <path d={d} stroke={`url(#${gId("v7gc")})`}
                  strokeWidth="2.5" fill="none" strokeLinecap="round"
                  opacity={op * 0.45} filter={`url(#${gId("v7pg")})`}
                />
              )}
              <path d={d} stroke={`url(#${gId("v7gc")})`}
                strokeWidth={isHot ? 1.4 : 0.9} fill="none"
        ...
        {labels.map(({ x, y, text }, i) => {
          ...
          return (
            <g key={i}>
              {/* Dot */}
              <circle cx={x} cy={y} r={isEnd ? 7 : 3.5}
                fill={col} opacity={isEnd ? 0.75 : op * 1.4}
              ...
              <text x={x + 12} y={y + 4}
                ...
                fill={dark ? "white" : "#1e1b4b"}
                opacity={op} fontFamily="monospace"
...
export function BackgroundV8({ preview = false }: { preview?: boolean }) {
  ...
  // ── Word pool ──────────────────────────────────────────────────────────────
  const painWords   = ["rejected","ghosted","no feedback","not a fit","overqualified",
                       "underqualified","position filled","not moving forward","regret to inform",
                       "culture fit","try again later","we'll keep your CV","no reply",
                       "automated rejection","thanks but no thanks","not selected"];

  const grindWords  = ["leetcode","system design","take-home task","SQL query","Big O",
                       "binary search","recursion","docker","CI/CD","pull request",
                       "code review","merge conflict","debug","unit tests","algorithms",
                       "data structures","REST API","microservices","git blame","refactor",
                       "TypeScript","React","Node.js","PostgreSQL","Redis","Kafka",
                       "behavioral round","STAR method","whiteboard","pair programming",
                       "HackerRank","Codility","dry run","time complexity","OOP"];

  const winWords    = ["offer!","you're hired","congrats","welcome aboard","start date",
                       "salary negotiated","sign the contract","accepted","new chapter",
                       "first day","team introduction","onboarding","badge access",
                       "first commit","new slack workspace","it worked!"];

  const neutralWords= ["apply","LinkedIn","cover letter","resume","portfolio","referral",
                       "recruiter call","phone screen","intro call","follow up","thank you note",
                       "second round","final round","reference check","background check",
                       "negotiate","counter offer","due diligence","headhunter","cold outreach",
                       "networking","job board","indeed","glassdoor","day 1","daily snap",
                       "progress","streak","keep going","one more day","almost there",
                       "you got this","stay consistent","track everything","log the snap"];
  ...
  allWords.forEach(({ text, cat }, i) => {
    ...
    const catBias = cat === "win" ? 0.65 : cat === "pain" ? 0.2 : cat === "grind" ? 0.45 : 0.4;
      ...
      <svg {...SVG}>
        <defs>
          <radialGradient id={gId("v8bg")} cx="85%" cy="5%" r="70%">
            <stop offset="0%"   stopColor={dark?"#0f172a":"#f8fafc"} stopOpacity={dark?0.7:0.5}/>
            <stop offset="100%" stopColor={dark?"#000":"#fff"}        stopOpacity="0"/>
          </radialGradient>
          <filter id={gId("v8glow")} x="-20%" y="-20%" width="140%" height="140%">
            <feGaussianBlur in="SourceGraphic" stdDeviation="2" result="b"/>
            <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge>
          </filter>
          <filter id={gId("v8fin")} x="-40%" y="-40%" width="180%" height="180%">
            <feGaussianBlur in="SourceGraphic" stdDeviation="4" result="b"/>
            <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge>
          </filter>
        </defs>
        ...
        {words.map(({ text, x, y, size, rot, cat }, i) => {
          ...
          const baseOp = dark
            ? (isWin ? 0.15 + norm * 0.5 : 0.06 + norm * 0.28)
            : (isWin ? 0.12 + norm * 0.38 : 0.05 + norm * 0.22);
          ...
          return (
            <text
              ...
              filter={isOffer ? `url(#${gId("v8fin")})` : isHot ? `url(#${gId("v8glow")})` : undefined}
...
export function BackgroundV9({ preview = false }: { preview?: boolean }) {
  ...
  const dust = Array.from({ length: 180 }, (_, i) => ({
    ...
    op: Math.abs(Math.cos(i * 71.4))  * (dark ? 0.18 : 0.1) + (dark ? 0.04 : 0.02),
  }));

  // ── Nodes: 3 zones, denser than V2 ───────────────────────────────────────
  const nodes = [
    // ── Zone 1: Dense start cluster (bottom-left) ──
    {x:45, y:870},{x:110,y:855},{x:185,y:875},{x:265,y:858},{x:355,y:868},
    {x:450,y:852},{x:545,y:862},{x:640,y:848},{x:740,y:857},{x:840,y:845},
    {x:945,y:853},{x:1050,y:841},{x:1155,y:849},{x:1265,y:838},{x:1375,y:845},
    {x:75, y:785},{x:175,y:770},{x:295,y:782},{x:415,y:768},{x:540,y:778},
    {x:665,y:762},{x:790,y:773},{x:915,y:758},{x:1040,y:768},{x:1165,y:752},
    {x:1295,y:762},{x:1400,y:750},
    {x:55, y:700},{x:170,y:688},{x:305,y:700},{x:445,y:684},{x:585,y:695},
    {x:725,y:678},{x:865,y:690},{x:1005,y:674},{x:1140,y:685},{x:1275,y:668},
    {x:1395,y:678},
    // ── Zone 2: Mid-journey spread ──
    {x:90, y:610},{x:225,y:596},{x:375,y:608},{x:530,y:592},{x:685,y:603},
    {x:840,y:586},{x:990,y:597},{x:1135,y:580},{x:1270,y:592},{x:1390,y:575},
    {x:120,y:515},{x:270,y:500},{x:435,y:512},{x:600,y:496},{x:765,y:508},
    {x:925,y:490},{x:1070,y:503},{x:1205,y:486},{x:1340,y:498},
    {x:155,y:420},{x:325,y:405},{x:505,y:418},{x:685,y:400},{x:860,y:414},
    {x:1020,y:396},{x:1170,y:410},{x:1305,y:392},{x:1410,y:405},
    // ── Zone 3: Sparse success cluster (top-right) ──
    {x:190,y:325},{x:380,y:310},{x:580,y:322},{x:780,y:305},{x:970,y:318},
    {x:1145,y:300},{x:1295,y:313},{x:1400,y:295},
    {x:240,y:228},{x:460,y:214},{x:695,y:226},{x:920,y:210},{x:1110,y:222},
    {x:1275,y:205},{x:1395,y:218},
    {x:320,y:130},{x:590,y:116},{x:870,y:128},{x:1090,y:112},{x:1285,y:124},
    {x:1400,y:108},
  ];

  // ── Edges: connect within threshold, coloured by zone ─────────────────────
  type Edge = { x1:number; y1:number; x2:number; y2:number; norm:number };
  const edges: Edge[] = [];
  for (let i = 0; i < nodes.length; i++) {
    for (let j = i + 1; j < nodes.length; j++) {
      const a = nodes[i]!;
  ...
  const hubs = [
    ...
    { x:445,  y:684, r:4,  label:"Polish CV",       sub:"craft your story",isEnd:false },
    { x:685,  y:603, r:4,  label:"First Apply",     sub:"100+ sent",       isEnd:false },
    { x:600,  y:496, r:4,  label:"Recruiter Call",  sub:"phone screen",    isEnd:false },
    { x:860,  y:414, r:4,  label:"Tech Interview",  sub:"code it live",    isEnd:false },
    { x:970,  y:318, r:4,  label:"System Design",   sub:"scale the world", isEnd:false },
    { x:1110, y:222, r:4,  label:"Final Round",     sub:"last hurdle",     isEnd:false },
    { x:1090, y:112, r:4,  label:"Ref. Check",      sub:"almost there",    isEnd:false },
    { x:1340, y:55,  r:7,  label:"Offer! 🎉",        sub:"you made it",     isEnd:true  },
  ];

  // ── Curved arcs between consecutive hubs ──────────────────────────────────
  // Each arc bulges perpendicular to the line between two hubs
  function hubArc(a: typeof hubs[0], b: typeof hubs[0], bulge: number): string {
  ...
  const arcs = hubs.slice(0, -1).map((h, i) => ({
    d:   hubArc(h, hubs[i + 1]!, (i % 2 === 0 ? 1 : -1) * 60),
      ...
      <svg {...SVG}>
        <defs>
          ...
          <radialGradient id={gId("v9bg")} cx="78%" cy="8%" r="72%">
            <stop offset="0%"   stopColor={c.bgA} stopOpacity={dark?0.65:0.5}/>
            <stop offset="60%"  stopColor={c.bgA} stopOpacity={dark?0.12:0.08}/>
            <stop offset="100%" stopColor={c.bgB} stopOpacity="0"/>
          </radialGradient>

          {/* Soft directional glow trail Start → Offer */}
          <linearGradient id={gId("v9trail")} x1="0%" y1="100%" x2="100%" y2="0%">
            <stop offset="0%"   stopColor={c.arc}  stopOpacity="0"/>
            <stop offset="50%"  stopColor={c.hub}  stopOpacity={dark?0.12:0.07}/>
            <stop offset="100%" stopColor={c.fin}  stopOpacity={dark?0.25:0.15}/>
          </linearGradient>
          ...
          {/* Finish glow */}
          <filter id={gId("v9fg")} x="-100%" y="-100%" width="300%" height="300%">
            <feGaussianBlur in="SourceGraphic" stdDeviation="10" result="b"/>
            <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge>
          </filter>
        ...
        {nodes.map(({ x, y }, i) => {
          ...
          const op  = dark
            ? (isHot ? 0.32 + norm * 0.25 : 0.1 + norm * 0.12)
            : (isHot ? 0.22 + norm * 0.2  : 0.07 + norm * 0.1);
          return (
          ...
          return (
            <g key={`h${i}`} filter={filt}>
              ...
              {[3.2, 2.1, 1.2].map((mult, ri) => (
                <circle key={ri} cx={x} cy={y} r={pr * mult}
                  fill="none" stroke={col} strokeWidth="0.8"
                  opacity={op * (0.15 - ri * 0.04)}/>
              ))}
              {/* Core dot */}
              <circle cx={x} cy={y} r={pr * 0.55}
                fill={col} opacity={Math.min(op * 1.1, 0.85)}/>
              {/* Label */}
              <text
